...
function Contracts({ data, actions }) {
  ...
  return <>
    <div className="top"><input className="input searchbar" placeholder="Rechercher un contrat..." value={q} onChange={e=>setQ(e.target.value)} /><div className="row"><button className="btn" onClick={()=>exportCSV("contrats.csv", [["Police","Client","Compagnie","Prime","Commission","Statut"], ...rows.map(c=>[c.police, clientName(data.clients.find(x=>x.id===c.client_id)), c.compagnie, c.prime, c.commission, c.statut])])}>Exporter CSV</button><button className="btn primary" onClick={()=>setOpen(true)}>+ Nouveau contrat</button></div></div>
    <div className="tablewrap"><table className="table"><thead><tr><th>Police</th><th>Client</th><th>Compagnie</th><th>Prime</th><th>Commission</th><th>Statut</th></tr></thead><tbody>{rows.map(c=>{ const cl = data.clients.find(x=>x.id===c.client_id); return <tr key={c.id}><td className="name">{c.police}</td><td>{clientName(cl)}</td><td>{c.compagnie}</td><td>{euro(c.prime)}</td><td>{euro(c.commission)}</td><td><Badge>{c.statut}</Badge></td></tr>; })}</tbody></table></div>
    {open && <ContractForm clients={data.clients} onClose={()=>setOpen(false)} onSave={async (payload)=>{ await actions.addContract(payload); setOpen(false); }} />}
  </>;
}

function ContractForm({ clients, onClose, onSave }) {
  const [form, setForm] = useState({ client_id: clients[0]?.id || "", police:"", compagnie:"", prime:"", commission:"", statut:"Actif" });
  const set = (k,v)=>setForm(s=>({...s,[k]:v}));
  return <Modal title="Nouveau contrat" onClose={onClose}>
    <div className="formgrid">
...
function Tasks({ data, actions }) {
  const [open, setOpen] = useState(false);
  return <>
    <div className="top"><div className="small">Clique pour terminer ou réouvrir une tâche.</div><button className="btn primary" onClick={()=>setOpen(true)}>+ Nouvelle tâche</button></div>
    <div className="row" style={{flexDirection:"column"}}>{data.tasks.map(t=>{ const cl = data.clients.find(c=>c.id===t.client_id); return <div className="card" key={t.id} style={{opacity:t.done?.7:1}}><div className="row" style={{justifyContent:"space-between", alignItems:"center"}}><div><div style={{fontWeight:800, textDecoration:t.done?"line-through":"none"}}>{t.titre}</div><div className="small">{clientName(cl)} · échéance {fmtDate(t.echeance)}</div></div><div className="row"><Badge>{t.priorite}</Badge><button className={`btn ${t.done?"":"good"}`} onClick={()=>actions.toggleTask(t)}>{t.done?"Réouvrir":"Terminer"}</button></div></div></div>;})}</div>
    {open && <TaskForm clients={data.clients} onClose={()=>setOpen(false)} onSave={async (payload)=>{ await actions.addTask(payload); setOpen(false); }} />}
  </>;
}

function TaskForm({ clients, onClose, onSave }) {
  const [form, setForm] = useState({ client_id: clients[0]?.id || "", titre:"", echeance:today(), priorite:"Normale", done:false });
  const set = (k,v)=>setForm(s=>({...s,[k]:v}));
  return <Modal title="Nouvelle tâche" onClose={onClose}>
    <div className="formgrid">
      <select className="select" value={form.client_id} onChange={e=>set("client_id",e.target.value)}>{clients.map(c=><option key={c.id} value={c.id}>{clientName(c)}</option>)}</select>
...
function Pipeline({ data, actions }) {
  ...
  const [open, setOpen] = useState(false);
  return <>
    <div className="top"><div className="small">Tu peux déplacer les opportunités entre les colonnes.</div><button className="btn primary" onClick={()=>setOpen(true)}>+ Opportunité</button></div>
    ...
    {open && <DealForm clients={data.clients} onClose={()=>setOpen(false)} onSave={async (payload)=>{ await actions.addDeal(payload); setOpen(false); }} />}
  </>;
}

function DealForm({ clients, onClose, onSave }) {
  const [form, setForm] = useState({ client_id: clients[0]?.id || "", titre:"", montant:"", stage:"lead" });
  const set = (k,v)=>setForm(s=>({...s,[k]:v}));
...
function Claims({ data, actions }) {
  const [open, setOpen] = useState(false);
  return <>
    <div className="top"><div className="small">Gestion simple des sinistres.</div><button className="btn primary" onClick={()=>setOpen(true)}>+ Nouveau sinistre</button></div>
    <div className="tablewrap"><table className="table"><thead><tr><th>Client</th><th>Description</th><th>Montant</th><th>Statut</th></tr></thead><tbody>{data.claims.map(c=>{ const cl = data.clients.find(x=>x.id===c.client_id); return <tr key={c.id}><td className="name">{clientName(cl)}</td><td>{c.description}</td><td>{euro(c.montant)}</td><td><Badge>{c.statut}</Badge></td></tr>; })}</tbody></table></div>
    {open && <ClaimForm clients={data.clients} onClose={()=>setOpen(false)} onSave={async (payload)=>{ await actions.addClaim(payload); setOpen(false); }} />}
  </>;
}

function ClaimForm({ clients, onClose, onSave }) {
  const [form, setForm] = useState({ client_id: clients[0]?.id || "", description:"", montant:"", statut:"Ouvert" });
  const set = (k,v)=>setForm(s=>({...s,[k]:v}));
  return <Modal title="Nouveau sinistre" onClose={onClose}>
    <div className="formgrid">
      <select className="select" value={form.client_id} onChange={e=>set("client_id",e.target.value)}>{clients.map(c=><option key={c.id} value={c.id}>{clientName(c)}</option>)}</select>
